...
enum LogLocator {
    private static var logDir: URL {
        ...
        return URL(fileURLWithPath: "/tmp/AGENT")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("AGENT-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("AGENT-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/AGENT/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("AGENT") && $0.pathExtension == "log" }
            .max { lhs, rhs in
