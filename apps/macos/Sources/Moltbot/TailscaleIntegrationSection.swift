...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.clawdbot/AGENT.json. Prefer CLAWDBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.clawdbot/AGENT.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.clawdbot/AGENT.json. Restart the gateway to apply."
        }
