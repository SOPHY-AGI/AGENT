...
struct MoltbotMacCLI {
    static func main() async {
        ...
        default:
            fputs("AGENT-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      AGENT-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      AGENT-mac discover [--timeout <ms>] [--json] [--include-local]
      AGENT-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      AGENT-mac connect
      AGENT-mac connect --url ws://127.0.0.1:18789 --json
      AGENT-mac discover --timeout 3000 --json
      AGENT-mac wizard --mode local
    """)
}
