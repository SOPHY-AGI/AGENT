...
IMAGE_NAME="${CLAWDBOT_IMAGE:-AGENT:local}"
...
echo ""
docker compose "${COMPOSE_ARGS[@]}" run --rm AGENT-cli onboard --no-install-daemon

echo ""
echo "==> Provider setup (optional)"
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm AGENT-cli providers login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm AGENT-cli providers add --provider telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm AGENT-cli providers add --provider discord --token <token>"
echo "Docs: https://docs.molt.bot/providers"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d AGENT-gateway
...
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f AGENT-gateway"
echo "  ${COMPOSE_HINT} exec AGENT-gateway node dist/index.js health --token \"$CLAWDBOT_GATEWAY_TOKEN\""
