...
Moltbot exposes OAuth expiry health via `AGENT models status`. Use that for
automation and alerting; scripts are optional extras for phone workflows.

## Preferred: CLI check (portable)

```bash
AGENT models status --check
```
...
- `scripts/claude-auth-status.sh` now uses `AGENT models status --json` as the
  source of truth (falling back to direct file reads if the CLI is unavailable),
  so keep `AGENT` on `PATH` for timers.
- `scripts/auth-monitor.sh`: cron/systemd timer target; sends alerts (ntfy or phone).
- `scripts/systemd/AGENT-auth-monitor.{service,timer}`: systemd user timer.
