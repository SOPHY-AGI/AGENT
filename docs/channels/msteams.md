...
```bash
AGENT plugins install @AGENT/msteams
```

Local checkout (when running from a git repo):
```bash
AGENT plugins install ./extensions/msteams
```
...
5. Configure `msteams` in `~/.clawdbot/AGENT.json` (or env vars) and start the gateway.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `AGENT-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `AGENT plugins install @AGENT/msteams`
   - From a local checkout: `AGENT plugins install ./extensions/msteams`
...
- CLI: `AGENT message poll --channel msteams --target conversation:<id> ...`
...
AGENT message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
AGENT message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
AGENT message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
AGENT message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
