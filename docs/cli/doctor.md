---
summary: "CLI reference for `AGENT doctor` (health checks + guided repairs)"
read_when:
...
- `--fix` (alias for `--repair`) writes a backup to `~/.clawdbot/AGENT.json.bak` and drops unknown config keys, listing each removal.
