...
```
[AGENT] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `AGENT status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch AGENT.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node AGENT.mjs status` works on Node 25.
