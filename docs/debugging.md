...
This is handy when you need to toggle obscure settings without editing `AGENT.json`.
...
```bash
pnpm gateway:dev
CLAWDBOT_PROFILE=dev AGENT tui
```

If you don’t have a global install yet, run the CLI via `pnpm AGENT ...`.
...
```bash
CLAWDBOT_PROFILE=dev AGENT gateway --dev --reset
```
