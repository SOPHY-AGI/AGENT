...
2) Put it on the **gateway host** (the machine running `AGENT gateway`).
...
API keys for daemon use: `AGENT onboard`.
...
```bash
AGENT models auth paste-token --provider anthropic
AGENT models auth paste-token --provider openrouter
```
...
Run `AGENT models status` to confirm which profile is expiring. If the profile
