---
summary: "All configuration options for ~/.clawdbot/AGENT.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Moltbot reads an optional **JSON5** config from `~/.clawdbot/AGENT.json` (comments + trailing commas allowed).
...
- Only diagnostic commands are allowed (for example: `AGENT doctor`, `AGENT logs`, `AGENT health`, `AGENT status`, `AGENT service`, `AGENT help`).
- Run `AGENT doctor` to see the exact issues.
- Run `AGENT doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `AGENT config set`. Keep a backup of `~/.clawdbot/AGENT.json`.
...
```bash
AGENT gateway call config.get --params '{}' # capture payload.hash
AGENT gateway call config.apply --params '{
...
```bash
AGENT gateway call config.get --params '{}' # capture payload.hash
AGENT gateway call config.patch --params '{
...
{ "$include": "/etc/AGENT/base.json5" } // absolute
...
- `~/.clawdbot/agent/*` (migrated by `AGENT doctor` into `~/.clawdbot/agents/<defaultAgentId>/agent/*`)
...
- Default log file: `/tmp/AGENT/AGENT-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/AGENT/AGENT.log`.
...
- The legacy single-account Baileys auth dir is migrated by `AGENT doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "AGENT", "clawd"] } }
    ]
  }
}
```
...
- `commands.config: true` enables `/config` (reads/writes `AGENT.json`).
...
Install it first: `AGENT plugins install @AGENT/mattermost` (or `./extensions/mattermost` from a git checkout).
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/AGENT" } }
}
```
...
`[AGENT]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[AGENT]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[AGENT]"`, Moltbot will instead use `[{identity.name}]` when the routed
...
- `tools.web.search.apiKey` (recommended: set via `AGENT configure --section web`, or use `BRAVE_API_KEY` env var)
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          containerPrefix: "AGENT-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
- Shortcut: `AGENT onboard --auth-choice opencode-zen`.
...
Shortcut: `AGENT onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `AGENT onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `AGENT onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `AGENT onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `AGENT onboard --auth-choice minimax-api`.
...
- Note: the control server is started by the running gateway (Moltbot.app menubar, or `AGENT gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/AGENT" }
...
Notes:
- `AGENT gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
...
macOS app behavior:
- Moltbot.app watches `~/.clawdbot/AGENT.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.clawdbot/AGENT.json` (or `CLAWDBOT_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.clawdbot/AGENT.json` (or `CLAWDBOT_CONFIG_PATH`)
...
- `AGENT --dev …` → uses `~/.clawdbot-dev` + shifts ports from base `19001`
- `AGENT --profile <name> …` → uses `~/.clawdbot-<name>` (port via config/env/flags)
...
Gmail helper config (used by `AGENT webhooks gmail setup` / `run`):
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__AGENT/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__AGENT__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
Controls LAN mDNS discovery broadcasts (`_AGENT-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_AGENT-bridge._tcp` under `~/.clawdbot/dns/` using the standard discovery domain `AGENT.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `AGENT.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `AGENT.internal` via that server
