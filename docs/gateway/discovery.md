...
The design goal is to keep all network discovery/advertising in the **Node Gateway** (`clawd` / `AGENT gateway`) and keep clients (mac app, iOS) as consumers.
...
- Service types:
  - `_AGENT-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `canvasPort=18793` (default canvas host port; serves `/__AGENT__/canvas/`)
  - `cliPath=<path>` (optional; absolute path to a runnable `AGENT` entrypoint or binary)
