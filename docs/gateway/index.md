...
- Replaces the legacy `gateway` command. CLI entry point: `AGENT gateway`.
...
pnpm gateway:watch
```
- Config hot reload watches `~/.clawdbot/AGENT.json` (or `CLAWDBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__AGENT__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `CLAWDBOT_SKIP_CANVAS_HOST=1`.
...
```bash
AGENT --dev setup
AGENT --dev gateway --allow-unconfigured
...
- `CLAWDBOT_CONFIG_PATH=~/.clawdbot-dev/AGENT.json`
...
```bash
AGENT --profile main gateway install
AGENT --profile rescue gateway install
```

Example:
```bash
CLAWDBOT_CONFIG_PATH=~/.clawdbot/a.json CLAWDBOT_STATE_DIR=~/.clawdbot-a AGENT gateway --port 19001
CLAWDBOT_CONFIG_PATH=~/.clawdbot/b.json CLAWDBOT_STATE_DIR=~/.clawdbot-b AGENT gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `AGENT health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `AGENT`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `AGENT gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.clawdbot.*` is cleaned up).
  - `AGENT doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Moltbot profile services.
  ...
  - Cleanup: `AGENT gateway uninstall` (current service) and `AGENT doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `AGENT gateway install --force` to reinstall (profile/env/path changes).
...
- To stop it cleanly, use `AGENT gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `AGENT gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `AGENT gateway install` first.
...
`AGENT gateway install` writes the user unit. `AGENT doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/AGENT-gateway[-<profile>].service`:
```
[Unit]
...
```
systemctl --user enable --now AGENT-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/AGENT-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now AGENT-gateway[-<profile>].service
```
...
## CLI helpers
- `AGENT gateway health|status` — request health/status over the Gateway WS.
- `AGENT message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `AGENT agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `AGENT gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `AGENT gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `AGENT gateway` and the legacy TCP control port.
