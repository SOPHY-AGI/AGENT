...
## Quick check: `AGENT security audit` (formerly `clawdbot security audit`)
...
- Tighten local perms (`~/.AGENT` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).
...
- **WhatsApp**: `~/.AGENT/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.AGENT/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.AGENT/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.AGENT/credentials/oauth.json`
...
`AGENT security audit` warns when this setting is enabled.
...
Moltbot stores session transcripts on disk under `~/.AGENT/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.AGENT` (see the audit section below). If you need
...
- If you install plugins from npm (`AGENT plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.AGENT/extensions/<pluginId>/` (or `$CLAWDBOT_STATE_DIR/extensions/<pluginId>/`).
...
AGENT pairing approve <channel> <code>
```
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.AGENT/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.AGENT or your logs.”
...
4. **Re-run audit**
   - `AGENT security audit --deep` and confirm the report is clean.
...
- `~/.AGENT/AGENT.json`: `600` (user read/write only)
- `~/.AGENT`: `700` (user only)

`AGENT doctor` can warn and offer to tighten these permissions.
...
The Gateway broadcasts its presence via mDNS (`_AGENT-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
Doctor can generate one for you: `AGENT doctor --generate-gateway-token`.
...
Assume anything under `~/.AGENT/` (or `$CLAWDBOT_STATE_DIR/`) may contain secrets or private data:

- `AGENT.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `AGENT status --all` (pasteable, secrets redacted) over raw logs.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `AGENT gateway` process.
...
1. Check Gateway logs: `/tmp/AGENT/AGENT-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.AGENT/agents/<agentId>/sessions/*.jsonl`.
