...
If the installer completes but `AGENT` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- Choose install method:
  - `npm` (default): `npm install -g AGENT@latest`
...
- If upgrading an existing install: runs `AGENT doctor --non-interactive` (best effort).
- For git installs: runs `AGENT doctor --non-interactive` after install/update (best effort).
...
This script installs `AGENT` into a prefix (default: `~/.clawdbot`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.
...
- Choose install method:
  - `npm` (default): `npm install -g AGENT@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `AGENT doctor --non-interactive` on upgrades and git installs (best effort).
...
```powershell
iwr -useb https://molt.bot/install.ps1 | iex -InstallMethod git -GitDir "C:\\AGENT"
```
...
- **"AGENT" is not recognized**: your npm global bin folder is not on PATH. Most systems use
