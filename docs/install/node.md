...
read_when:
  - "You installed Moltbot but `AGENT` is “command not found”"
...
If you can run `npm install -g AGENT@latest` but later see `AGENT: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
