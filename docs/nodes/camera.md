...
```bash
AGENT nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
AGENT nodes camera snap --node <id> --facing front
AGENT nodes camera clip --node <id> --duration 3000
AGENT nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`AGENT.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `AGENT` CLI to invoke camera commands on the macOS node.

Examples:

```bash
AGENT nodes camera list --node <id>            # list camera ids
AGENT nodes camera snap --node <id>            # prints MEDIA:<path>
AGENT nodes camera snap --node <id> --max-width 1280
AGENT nodes camera snap --node <id> --delay-ms 2000
AGENT nodes camera snap --node <id> --device-id <id>
AGENT nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
AGENT nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
AGENT nodes camera clip --node <id> --device-id <id>
AGENT nodes camera clip --node <id> --no-audio
```

Notes:
- `AGENT nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
AGENT nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
