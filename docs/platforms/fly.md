...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `AGENT.json` where they could be accidentally exposed or logged.
...
**Note:** With `CLAWDBOT_STATE_DIR=/data`, the config path is `/data/AGENT.json`.
...
Or visit `https://my-AGENT.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/AGENT.json` should be read on restart.

Verify the config exists:
```bash
fly ssh console --command "cat /data/AGENT.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/AGENT.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/AGENT.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:
```bash
fly ssh console --command "rm /data/AGENT.json"
```
...
```bash
# List current IPs
fly ips list -a my-AGENT

# Release public IPs
fly ips release <public-ipv4> -a my-AGENT
fly ips release <public-ipv6> -a my-AGENT
...
fly ips allocate-v6 --private -a my-AGENT
```
...
fly proxy 3000:3000 -a my-AGENT
