---
summary: "Direct `AGENT agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `AGENT agent` (direct agent runs)

`AGENT agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `AGENT message --target`).
...
```bash
AGENT agent --to +15555550123 --message "status update"
...
AGENT agent --to +15555550123 --message "Summon reply" --deliver
