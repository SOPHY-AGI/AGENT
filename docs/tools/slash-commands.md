...
- `commands.config` (default `false`) enables `/config` (reads/writes `AGENT.json`).
...
- For full provider usage breakdown, use `AGENT status --usage`.
...
```
/debug show
/debug set messages.responsePrefix="[AGENT]"
...
Notes:
- Overrides apply immediately to new config reads, but do **not** write to `AGENT.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`AGENT.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[AGENT]"
