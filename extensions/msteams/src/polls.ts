...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["AGENTPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["AGENT", "pollId"]) ??
    readNestedString(value, ["AGENT", "poll", "id"]) ??
    readNestedString(value, ["data", "AGENTPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "AGENT", "pollId"]);
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "AGENT poll vote",
          displayText: "Vote recorded",
          value: { AGENTPollId: pollId },
        },
      },
    ],
  };
