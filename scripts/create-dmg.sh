...
DMG_TEMP="$(mktemp -d /tmp/AGENT-dmg.XXXXXX)"
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/AGENT-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/AGENT-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/AGENT-dmg-limits.txt
