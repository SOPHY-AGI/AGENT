...
IMAGE_NAME="AGENT-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/AGENT-bin:$PATH"
  mkdir -p /tmp/AGENT-bin

  cat > /tmp/AGENT-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/AGENT-bin/systemctl

  cat > /tmp/AGENT-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/AGENT-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/AGENT"
  npm_entry="/tmp/npm-prefix/lib/node_modules/AGENT/AGENT.mjs"
  git_entry="/app/AGENT.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/AGENT-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/AGENT-gateway.service"
    if [ ! -f "$unit_path" ]; then
