...
BASE_IMAGE="${BASE_IMAGE:-AGENT-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-AGENT-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=AGENT.sandbox=1)
NOTE
