...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", `AGENT.sessionKey=${params.scopeKey}`);
  args.push("--label", `AGENT.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `AGENT.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "AGENT.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`AGENT sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`AGENT sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("AGENT sandbox recreate --all");
}
