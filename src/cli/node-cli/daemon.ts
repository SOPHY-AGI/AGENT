...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("AGENT node install"), formatCliCommand("AGENT node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("AGENT node install --force")}`);
    }
    return;
  }
