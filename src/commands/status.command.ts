...
export async function statusCommand(
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("AGENT security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("AGENT security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("AGENT status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("AGENT logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("AGENT status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("AGENT gateway probe")}`);
  }
}
