...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "AGENT-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(home: string, dirname: ".AGENT" | ".clawdbot", port: number) {
...
describe("config io compat (new + legacy folders)", () => {
  it("prefers ~/.AGENT/AGENT.json when both configs exist", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".AGENT", 19001);
  ...
  it("falls back to ~/.clawdbot/AGENT.json when only legacy exists", async () => {
    await withTempHome(async (home) => {
  ...
  it("honors explicit legacy config path env override", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".AGENT", 19002);
