...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.CLAWDBOT_SERVICE_MARKER).toBe("AGENT");
    expect(env.CLAWDBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.CLAWDBOT_SERVICE_VERSION).toBe("string");
    expect(env.CLAWDBOT_SYSTEMD_UNIT).toBe("AGENT-gateway.service");
  ...
  it("uses profile-specific unit and label", () => {
    ...
    expect(env.CLAWDBOT_SYSTEMD_UNIT).toBe("AGENT-gateway-work.service");
    if (process.platform === "darwin") {
