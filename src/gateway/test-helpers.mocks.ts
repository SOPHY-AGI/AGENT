...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `AGENT-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.CLAWDBOT_CONFIG_PATH = path.join(root, "AGENT.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "AGENT.json");
