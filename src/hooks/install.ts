...
async function ensureMoltbotHooks(manifest: HookPackageManifest) {
  const hooks = manifest.AGENT?.hooks ?? manifest[LEGACY_MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing AGENT.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json AGENT.hooks is empty");
  }
  return list;
}
...
export async function installHooksFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "AGENT-hook-"));
...
export async function installHooksFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "AGENT-hook-pack-"));
