...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[AGENT] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[AGENT] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }
    ...
    if (isTransientNetworkError(reason)) {
      console.error("[AGENT] Network error (non-fatal):", formatUncaughtError(reason));
      return;
    }

    console.error("[AGENT] Unhandled promise rejection:", formatUncaughtError(reason));
    process.exit(1);
  });
}
