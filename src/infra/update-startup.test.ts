...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "AGENT-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveMoltbotPackageRoot } = await import("./AGENT-root.js");
    ...
    vi.mocked(resolveMoltbotPackageRoot).mockResolvedValue("/opt/AGENT");
