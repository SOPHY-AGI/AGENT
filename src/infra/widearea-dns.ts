...
export const WIDE_AREA_DISCOVERY_DOMAIN = "AGENT.internal.";
export const WIDE_AREA_ZONE_FILENAME = "AGENT.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*AGENT-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "AGENT";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "AGENT");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "AGENT-gw");
  ...
  records.push(`_AGENT-gw._tcp IN PTR ${instanceLabel}._AGENT-gw._tcp`);
  records.push(`${instanceLabel}._AGENT-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._AGENT-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; AGENT-content-hash: ${contentHash}\n${contentBody}`;
}
