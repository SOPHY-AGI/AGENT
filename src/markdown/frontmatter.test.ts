...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { AGENT?: { emoji?: string } };
    expect(parsed.AGENT?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { AGENT?: { events?: string[] } };
