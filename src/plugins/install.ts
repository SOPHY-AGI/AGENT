...
async function ensureMoltbotExtensions(manifest: PackageManifest) {
  const extensions = manifest.AGENT?.extensions ?? manifest[LEGACY_MANIFEST_KEY]?.extensions;
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing AGENT.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json AGENT.extensions is empty");
  }
  return list;
}
...
export async function installPluginFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "AGENT-plugin-"));
...
export async function installPluginFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "AGENT-npm-pack-"));
